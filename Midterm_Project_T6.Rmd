...
transcript_data_types <- sapply(transcript, class)
print(transcript_data_types)
```
```{r}
...
```{r}
#Rename columns 
colnames(transcript)[colnames(transcript) == "time"] <- "days_elapsed"
colnames(transcript)[colnames(transcript) == "person"] <- "customer_id"
tail(transcript)
```
