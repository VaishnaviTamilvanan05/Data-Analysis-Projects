...
**Observations:** The customer base consists of a larger proportion of males (57.2%) compared to females (41.3%), with a minor representation (1.4%) from customers identifying with other genders.
...
overall_complete_rate <- p2 / p0

# Sort the overall completion rate in descending order
overall_complete_rate <- overall_complete_rate[order(overall_complete_rate, decreasing = TRUE)]
...
completion_data <- data.frame(
  Offer = factor(names(overall_complete_rate), levels = names(overall_complete_rate)[order(overall_complete_rate, decreasing = TRUE)]),
  CompletionRate = overall_complete_rate[order(overall_complete_rate, decreasing = TRUE)] # Sort the data by CompletionRate in descending order
...
# Create a bar plot
barplot(p0, names.arg = names(p0), col = 'orange', xlab = 'Offer', ylab = 'Count',
        main = 'Offer Received', cex.main = 1.8, cex.lab = 1.5, cex.axis = 1.2)

# Rotate x-axis labels for better readability
#axis(1, labels = names(p0), las = 2)
...
barplot(p1_rate, col = c('#011f4b', '#011f4b', '#011f4b', '#011f4b', '#03396c', '#03396c', '#005b96', '#005b96', '#005b96', '#b3cde0'),
        ...
        ylim = c(0, 1.2),
        yaxt = 'n')
axis(2, at = seq(0, 1.2, 0.2), labels = paste0(seq(0, 1, 0.2) * 100, '%'))
...
The above observations shows that the variables reward and difficulty , reward and duration had a positive correlation indicating that with increase in reward, duration and the difficulty will also increase.

Difficulty and reward also seems to have a positive correlation indicating that increase in difficulty tends to increase the reward.
...
```
OBSERVATION:
The above observation shows that the combination of 3 channels had the maximum count per offer.

## MERGING ALL THE CSV FILES FOR BETTER RESULTS

```{r}
# df_offers and df_cust_offers
merged_df <- merge(df_cust_offer, df_offers, by.x = "value_data", by.y = "id")
head(merged_df)

#customers and merged_df
final <- merge(merged_df, customers, by.x = "customer_id", by.y = "id")
head(final)

```
```{r}
# Counting the frequency of each offer type
offer_counts <- table(final_merged_df$offer_type)
...
To conclude, based on the completions, discount offer had an upper hand as it attracted many customers.

```{r}
#Offer Type Preferences by Gende
...
OBSERVATIONS:
This indicates the engagement patterns with different offer types such as BOGO, Discount, and Informational offers for different gender categories(Female,male, other)
